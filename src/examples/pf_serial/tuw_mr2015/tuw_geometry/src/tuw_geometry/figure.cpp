...
void Figure::init () {
    ...
    ox_ = -(min_x_+max_x_)/2; // offset image space x
    oy_ = -(min_y_+max_y_)/2; // offset image space y
    mx_ = width_pixel_/2; // visual image space x
    my_ = height_pixel_/2; // visual image space y
    
    
    
    cv::Matx<double, 3, 3 > Tw ( 1, 0, ox_,
				 0, 1, oy_,
